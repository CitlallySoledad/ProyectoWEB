...
class EventController extends Controller
{
    ...
    public function update(UpdateEventRequest $request, Event $event)
    {
        ...
        // Actualizar automáticamente la rúbrica en todos los proyectos enviados de este evento
        if (!empty($newRubricIds)) {
            $newRubricId = is_array($newRubricIds) ? $newRubricIds[0] : $newRubricIds;
            
            \App\Models\Project::where('event_id', $event->id)
                ->where('status', 'enviado')
                ->update(['rubric_id' => $newRubricId]);
        }

        return redirect()
            ->route('admin.events.index')
            ->with('success', 'Evento y proyectos actualizados correctamente.');
    }
